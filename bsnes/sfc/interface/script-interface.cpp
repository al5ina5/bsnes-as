...
#define REG_LAMBDA_BEHAVIOUR(name, bhvr, defn, lambda) r = e->RegisterObjectBehaviour(#name, bhvr, defn, asFUNCTION(+lambda), ((bhvr == asBEHAVE_RELEASE || bhvr == asBEHAVE_ADDREF) ? asCALL_CDECL_OBJLAST : (bhvr == asBEHAVE_CONSTRUCT) ? asCALL_CDECL_OBJFIRST : asCALL_CDECL)); assert( r >= 0 )
